...
const componentName = 'EditIncomeComponent';

describe(`${moduleName}.${componentName}`, () => {
  let testTarget: EditIncomeComponent;
  ...
  beforeEach(() => {
    ...
    testTarget = new EditIncomeComponent(authServiceMock, activatedRouterMock, routerMock, messageMock, storeMock);
  });

  describe('#ngOnInit', () => {
    it('should not call messageMock.open()', () => {
      // Arrange
      storeMock.select = sinon.stub().returns(of({incomeById: 10}));
    ...
    it('should call message.open() with parameter class success and router.navigate() with ["/incomes/all"]', () => {
      // Arrange
      storeMock.select = sinon.stub().returns(of({incomeById: false, isAdded: true}));
      const testObj = {
        panelClass: 'my-custom-container',
        data: {
          content: 'The income was changed successfully', class: 'success', time: 800
        }
      };
      ...
      sinon.assert.calledWith(routerMock.navigate, ['/incomes/all']);
    });
    ...
    it('should call message.open() with parameter class error', () => {
      ...
      const testObj = {
        panelClass: 'my-custom-container',
        data: {
          content: 'Error, the income was not changed', class: 'error', time: 800
        }
      };
